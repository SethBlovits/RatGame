...
public class LobbyManager : NetworkBehaviour
{
    ...
    public NetworkList<PlayerInfo> allNetPlayers; // PlayerInfo is a struct for each player that contains a clientId, their name, and ready status
...
//Colors for users
    ...
    private void Start()
    {
        startBttn.onClick.AddListener(startButtonClick);
        
        if (IsHost)
        {
            foreach (NetworkClient nc in NetworkManager.ConnectedClientsList) //when scene starts collect all the connected clients
            {
               
                AddPlayerToList(nc.ClientId); //add each network client the network list that contains their ulongs
            }
